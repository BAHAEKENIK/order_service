...
// ... (Public routes, Guest Auth routes, Client routes - no changes from previous) ...
...
Route::middleware('guest')->group(function () {
    Route::get('login', [LoginController::class, 'create'])->name('login'); Route::post('login', [LoginController::class, 'store'])->name('login.store');
    Route::get('register', [RegisterController::class, 'create'])->name('register'); Route::post('register', [RegisterController::class, 'storeUser'])->name('register.storeUser');
    Route::get('provider-registration', [RegisterController::class, 'showProviderRegistrationForm'])->name('provider.register.form'); Route::post('provider-registration', [RegisterController::class, 'storeProviderDetails'])->name('provider.register.store');
    Route::get('forgot-password', [ForgotPasswordController::class, 'create'])->name('password.request'); Route::post('forgot-password', [ForgotPasswordController::class, 'store'])->name('password.email');
    Route::get('reset-password/{token}', [ResetPasswordController::class, 'create'])->name('password.reset'); Route::post('reset-password', [ResetPasswordController::class, 'store'])->name('password.update');
});
Route::post('logout', [LoginController::class, 'destroy'])->name('logout')->middleware('auth');

Route::middleware(['auth'])->group(function () {
    Route::get('/dashboard', function () { $user = Auth::user(); if ($user->isAdmin()) { return redirect()->route('admin.dashboard'); } if ($user->isProvider()) { return redirect()->route('provider.requests.index'); } return redirect()->route('client.requests.my'); })->name('dashboard');
    Route::prefix('client')->name('client.')->group(function () { /* ... client routes as before ... */
    ...
    Route::prefix('provider')->name('provider.')->group(function () {
        ...
        Route::get('/my-services', [ProviderDashboardController::class, 'myServices'])->name('services.index');
        Route::get('/my-services/create', [ProviderDashboardController::class, 'createService'])->name('services.create');
        Route::post('/my-services', [ProviderDashboardController::class, 'storeService'])->name('services.store');
        Route::get('/my-services/{service}/edit', [ProviderDashboardController::class, 'editService'])->name('services.edit');
        Route::put('/my-services/{service}', [ProviderDashboardController::class, 'updateService'])->name('services.update');
        Route::delete('/my-services/{service}', [ProviderDashboardController::class, 'destroyService'])->name('services.destroy');
        Route::get('/inbox', [ProviderDashboardController::class, 'inbox'])->name('messages.index');
        Route::get('/inbox/chat/{serviceRequest}', [ProviderDashboardController::class, 'showChat'])->name('messages.chat');
        Route::post('/inbox/chat/{serviceRequest}', [ProviderDashboardController::class, 'storeMessage'])->name('messages.store');

        // UPDATED Route for provider to contact admin
        Route::get('/contact-admin', function() {
            $admin = \App\Models\User::where('role', 'admin')->first(); // Assuming one admin
            if(!$admin) {
                return redirect()->route('provider.messages.index')->with('error', 'Admin not found.');
            }
            $controller = app(ProviderDashboardController::class);
            return $controller->chatWithAdmin($admin); // Call the new method
        })->name('contact.admin');

        Route::get('/profile', [ProviderDashboardController::class, 'profile'])->name('profile.edit');
        Route::post('/profile', [ProviderDashboardController::class, 'updateProfile'])->name('profile.update');
        Route::delete('/profile', [ProviderDashboardController::class, 'destroyAccount'])->name('profile.destroy');
        Route::get('/reviews', [ProviderDashboardController::class, 'reviews'])->name('reviews.index');
    });

    Route::prefix('admin')->name('admin.')->group(function () {
        Route::get('/dashboard', function () { return "Welcome Admin to your Dashboard!"; })->name('dashboard');
    });
});
