...
<head>
    ...
    <title>@yield('title', 'Admin Dashboard') - Easy Services</title>
    ...
    <style>
        ...
        .notification-dot { position: absolute; top: 0.6rem; right: 0.6rem; width: 8px; height: 8px; background-color: #EF4444; border-radius: 50%;}
    ...
    <div class="dashboard-layout">
        <aside class="sidebar" id="adminSidebar">
            ...
            <nav class="sidebar-nav flex-grow">
                <ul>
                    ...
                    <li>
                        <a href="{{ route('admin.contact-messages.index') }}" class="relative {{ request()->routeIs('admin.contact-messages.*') ? 'active' : '' }}">
                            <i class="fas fa-envelope-open-text"></i> Contact Form Inbox
                            @php $newContactMessagesCount = \App\Models\ContactUsMessage::where('status', 'new')->count(); @endphp
                            @if($newContactMessagesCount > 0)
                                <span class="notification-dot" title="{{ $newContactMessagesCount }} new message{{ $newContactMessagesCount > 1 ? 's' : '' }}"></span>
                            @endif
                        </a>
                    </li>
                     <li>
                        <a href="{{ route('admin.inbox.index') }}" class="relative {{ request()->routeIs('admin.inbox.*') ? 'active' : '' }}">
                            <i class="fas fa-headset"></i> User Support Chats
                             @php
                                $adminSupportUnread = \App\Models\Message::where('receiver_id', Auth::id())
                                    ->whereNull('read_at')
                                    ->whereHas('serviceRequest', function($q) {
                                        // Assumes 'admin-support' category or specific description pattern identifies these
                                        $q->whereHas('category', fn($cat) => $cat->where('slug', 'admin-support')
                                                                             ->orWhere('slug', 'admin-user-direct-chat')
                                           )
                                          ->orWhere('description', 'like', 'Support chat with Admin%');
                                    })->count();
                            @endphp
                            @if($adminSupportUnread > 0)
                                <span class="notification-dot" title="{{ $adminSupportUnread }} unread support message{{ $adminSupportUnread > 1 ? 's' : '' }}"></span>
                            @endif
                        </a>
                    </li>
                </ul>
            ...
            <div class="sidebar-footer">
                <div class="user-profile-widget">
                    <a href="{{ route('admin.profile.edit') }}">
                        <img src="{{ Auth::user()->profile_photo_path ? Storage::url(Auth::user()->profile_photo_path) : 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name).'&color=FFFFFF&background=4A55A2' }}" alt="{{ Auth::user()->name }}">
                    </a>
                    <div class="mr-auto">
                        <p class="font-semibold">{{ Auth::user()->name }}</p>
                        <a href="{{ route('admin.profile.edit') }}" class="view-profile-link">Admin Settings</a>
                    </div>
                </div>
            </div>
        </aside>
        ...
        <div class="main-content-wrapper">
            <header class="content-header">
                <h1 class="content-title">@yield('page-title', 'Admin Panel')</h1>
                <div class="top-bar-actions">
                     <button id="theme-toggle-dashboard" title="Toggle Theme">
                        <i class="fas fa-sun"></i><i class="fas fa-moon" style="display:none;"></i>
                    </button>
                    <form method="POST" action="{{ route('logout') }}" class="inline"> @csrf <button type="submit" class="logout-btn" title="Logout">LOGOUT</button> </form>
                </div>
            </header>
            <main class="main-content">
                @if(session('success')) <div style="background-color: #D1FAE5; color: #065F46; padding: 1rem; border-radius: 0.375rem; margin-bottom: 1.5rem; font-size:0.9rem;">{{ session('success') }}</div> @endif
                @if(session('error')) <div style="background-color: #FEE2E2; color: #991B1B; padding: 1rem; border-radius: 0.375rem; margin-bottom: 1.5rem; font-size:0.9rem;">{{ session('error') }}</div> @endif
                @if(session('info')) <div style="background-color: #DBEAFE; color: #1E40AF; padding: 1rem; border-radius: 0.375rem; margin-bottom: 1.5rem; font-size:0.9rem;">{{ session('info') }}</div> @endif
                @yield('content')
            </main>
        </div>
    </div>
<script> /* Theme toggle JS */ const themeToggleDashboard = document.getElementById('theme-toggle-dashboard'); const body = document.body; const applyDashboardTheme = (theme) => { const sunIcon = themeToggleDashboard?.querySelector('.fa-sun'); const moonIcon = themeToggleDashboard?.querySelector('.fa-moon'); if (theme === 'dark') { body.classList.add('dark-mode'); document.documentElement.classList.add('dark'); if(sunIcon) sunIcon.style.display='none'; if(moonIcon) moonIcon.style.display='inline'; } else { body.classList.remove('dark-mode'); document.documentElement.classList.remove('dark'); if(sunIcon) sunIcon.style.display='inline'; if(moonIcon) moonIcon.style.display='none'; }}; const savedTheme = localStorage.getItem('theme'); const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches; if (savedTheme) { applyDashboardTheme(savedTheme); } else if (prefersDark) { applyDashboardTheme('dark'); } else { applyDashboardTheme('light'); } if(themeToggleDashboard) { themeToggleDashboard.addEventListener('click', () => { const isDarkMode = body.classList.contains('dark-mode'); if (isDarkMode) { applyDashboardTheme('light'); localStorage.setItem('theme', 'light'); } else { applyDashboardTheme('dark'); localStorage.setItem('theme', 'dark'); }});} </script>
@stack('scripts')
</body>
</html>
